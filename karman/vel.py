...
from ufl import as_vector, curl, inner, CellVolume
...
class NavierStokesSolver:
    def __init__(self, gridView, order, mu, rho, dt, max_refinement_level=5, load_data=False):
        ...
        self.setup_space(load_data=load_data)
    ...
    def setup_space(self, load_data=False):
        ...
        self.difference_velocity = ufl.as_vector(
            [
                self.solution_u[0] - self.u_old[0],
                self.solution_u[1] - self.u_old[1],
            ]
        )
        if load_data:
            try:
    ...
    def adapt(self):
        marker=self.velocity_marker
        marker.as_numpy[:] = np.sqrt(marker.as_numpy)
        max_marker = comm.max(np.max(marker.as_numpy))
        min_marker = comm.min(np.min(marker.as_numpy))
        if max_marker > min_marker:
    ...
    def calc_curl(self):
        @fem.function.gridFunction(self.gridView, codim=0, order = 0, name="volume")
        def volume(element, x):
            return element.geometry.volume
            
        curl_vec = curl(as_vector([self.solution_u[0], self.solution_u[1]]))
        curl_mag_sq = inner(curl_vec, curl_vec)
        cell_vol = volume
        curl_integral = self.element_storage.interpolate(
            curl_mag_sq * cell_vol, name="curl_integral"
        )
        curl_array = curl_integral.as_numpy
        min_curl = comm.min(np.min(np.abs(curl_array)))
        max_curl = comm.max(np.max(np.abs(curl_array)))

        if max_curl > min_curl:
            curl_array[:] -= min_curl
            curl_array[:] /= (max_curl - min_curl)

        print(f"max curl: {max_curl}, min curl: {min_curl}")
        return curl_integral
    ...
    def perform_one_step(self):
        ...
        delta_v = ufl.inner(self.difference_velocity, self.difference_velocity)
        self.velocity_marker = self.element_storage.interpolate(delta_v, name="marker")
    ...
    def integrate(self, endTime, time_between_plots=None):
        next_plot_time = 0
        progress_bar = tqdm(total=endTime, desc="Integration Progress" + str(comm.size))
        while self.t.value < endTime:
            self.t.value += self.dt.value
            self.perform_one_step()
            
            if time_between_plots is not None and self.t.value >= next_plot_time:
                progress_bar.update(time_between_plots) if comm.rank == 0 else None
