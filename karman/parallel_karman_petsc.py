...
class NavierStokesSolver:
    def __init__(self, gridView, order, mu, rho, dt, max_refinement_level=5):
        ...
        self.init()
        self.vtk = self.gridView.sequencedVTK("karman", pointdata=[self.composite_solution])
        self.max_refinement_level = max_refinement_level

    def init(self):
        self.V_space = lagrange(self.gridView, order=self.order, dimRange=self.gridView.dimension, storage="petsc") # p1Bubble als alternative
        # pressure space
    ...
    def generate_navier_stokes_schemes(self, velocity_boundary_condition, pressure_boundary_condition, neumann_boundary_form, preconditioner="gauss-seidel"):
        ...
        params = {"nonlinear.verbose": True,
                  "linear.verbose": True,
        }
        self.step_one_scheme = fem.scheme.galerkin([step_one_form == 0, *velocity_boundary_condition], solver="gmres", parameters={"linear.preconditioner.method": preconditioner})
        self.step_two_scheme = fem.scheme.galerkin([step_two_form == 0, *pressure_boundary_condition], solver="gmres", parameters={"linear.preconditioner.method": preconditioner})
        self.step_three_scheme = fem.scheme.galerkin([step_three_form == 0, *velocity_boundary_condition], solver="gmres", parameters={"linear.preconditioner.method": preconditioner})
    ...
    def integrate(self, endTime, time_between_plots = None):
        ...
        while self.t.value < endTime:
            ...
            if time_between_plots is not None and self.t.value >= next_plot_time:
                ...
                self.vtk()
                #self.adapt()
                dune.fem.loadBalance([self.composite_solution, self.u_old, self.p_old])
...
vortex_street_grid.hierarchicalGrid.globalRefine(3)
mu.value = 1E-3
dt.value = 5E-5
...
print("Calculating initial velocity and pressure field with quasi stokes")
...
params = {"nonlinear.verbose": True,
                  "linear.verbose": True,
                  "linear.preconditioner.method": "gauss-seidel",
        }
